 ...
 public:
  auto acquire() {
    struct mutex_lock_operation {
      ...
      bool await_ready() const noexcept { return !m_mutex->is_locked(); }
      void await_suspend(std::coroutine_handle<> handle) noexcept {
        m_handle = handle;
        m_mutex->m_continuation.push(handle);
      }
      auto await_resume() const noexcept { return locked_mutex(*m_mutex); }
      void await_cancel() noexcept { m_mutex->m_continuation.erase(m_handle); }
